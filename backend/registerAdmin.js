...
async function registerAdminUser() {
    try {
        ...
        const DMSadmin = process.argv[2];
        const userExists = await wallet.get(DMSadmin);
        if (userExists) {
            console.log(`An identity for the user ${DMSadmin} already exists in the wallet`);
            return;
        }
        ...
        console.log(`Successfully registered user ${DMSadmin} with enrollment secret ${enrollmentSecret}`);
